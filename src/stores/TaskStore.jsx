  ...
  persist(
    (set) => ({
      ...
      fetchTasks: async () => {
        try {
          const response = await fetch(
            "http://localhost:8080/project5/rest/users/tasks",
            {
              method: "GET",
              headers: {
      ...
      fetchTasksByUser: async (username) => {
        try {
          const response = await fetch(
            `http://localhost:8080/project5/rest/users/${username}/tasks`,
            {
              method: "GET",
              headers: {
                "Content-Type": "application/json",
      ...
      fetchTasksByCategory: async (category) => {
        try {
          const response = await fetch(
            `http://localhost:8080/project5/rest/users/tasks/${category}`,
            {
              method: "GET",
              headers: {
                "Content-Type": "application/json",
      ...
      deleteAllUserTasks: async (username) => {
        try {
          const response = await fetch(
            `http://localhost:8080/project5/rest/users/eraseAllTasks/${username}`,
            {
              method: "PUT",
              headers: {
                "Content-Type": "application/json",
          ...
          if (response.ok) {
            ...
            console.error(
              "Failed to delete all user tasks:",
              response.statusText
            );
          }
        } catch (error) {
